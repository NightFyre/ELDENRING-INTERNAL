...
bool ImGui::BitField(const char* label, uint8_t* bits, unsigned* hoverIndex, std::string* hoverTexts)
{
    ...
    for (unsigned i = 0; i < 8; ++i)
    {
        ...
        if (hovered && hoverIndex) {
            *hoverIndex = i;
            if(!hoverTexts[i].empty())
                SetTooltip(hoverTexts[i].c_str());
        }
...
bool ImGui::Toggle(const char* label, bool* v)
{
    ...
    window->DrawList->AddRectFilled(newRect.Min, newRect.Max, ImColor(52, 61, 70), 10);      //  OFF COLOR
    ...
    window->DrawList->AddRectFilled(sRect.Min, sRect.Max, ImColor(255, 255, 255), 10);      //  DIAL  COLOR
    ...
    if (*v) {
        ...
        //reset
        window->DrawList->AddRectFilled(ImVec2(newRect.Min.x - 1.f, newRect.Min.y - 1.f), ImVec2(newRect.Max.x + .5f, newRect.Max.y + 1.f), ImColor(191, 191, 74/*58, 159, 191*/), 10); //  ON COLOR

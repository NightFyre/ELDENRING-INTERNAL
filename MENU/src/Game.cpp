...
namespace ER {
	...
	void GameFunctions::FMVSkip(uintptr_t addr)
	{
		...
		using namespace ER;
		Memory::Patch((BYTE*)addr + g_GameVariables->offsets.ptr_FMV_SKIP, (BYTE*)"\xE9\x1C\x00\x00\x00", 5);
	...
	{
		using namespace ER;
		Memory::Patch((BYTE*)addr + g_GameVariables->offsets.ptr_UNLOCK_FPS, (BYTE*)"\xC7\x45\xEF\x00\x00\x00\x00", 7);  // AOB 2 | OLD 0x1944B27
	...
	{
		switch (ACTIVE) {
			case (TRUE): Memory::Patch((BYTE*)addr + g_GameVariables->offsets.ptr_PAUSE_GAME, (BYTE*)"\x85", 1); break;
			case (FALSE): Memory::Patch((BYTE*)addr + g_GameVariables->offsets.ptr_PAUSE_GAME, (BYTE*)"\x84", 1); break;
		}	
	}
